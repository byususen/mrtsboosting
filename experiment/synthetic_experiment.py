...
from mrtsboosting.experiment import rstsf
...
for scenario_name, prop_cloud in scenarios.items():
    ...
    for q in [0, 1]:
        ...
        for r in range(start_iter, start_iter + n_iter):
            ...
            # Run all models
            models = {
                'mrtsboost': MRTSBoostingClassifier(random_state=SEED),
                'tsf': TimeSeriesForest(random_state=SEED),
                'rocket': RocketClassifier(random_state=SEED),
                'drcif': DrCIFClassifier(random_state=SEED),
                'dtw': KNeighborsTimeSeriesClassifier(n_neighbors=1, metric="dtw"),
                'rstsf': rstsf(),
                'hc2': HIVECOTEV2(random_state=SEED),
                'boss': BOSSEnsemble(random_state=SEED),
                'st': ShapeletTransformClassifier(random_state=SEED)
            }
