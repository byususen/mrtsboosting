...
def run_all_models(X_train, y_train, X_test, y_test, dataset_name):
    global results
    ...
    try:
        ...
        X_train_df.set_index(['id_sample', 'id_time'], inplace=True)
        
        X_train_arr = convert_to(X_train_df, to_type="numpy3D")
        
        #X_train_nested = convert(X_train_arr, from_type="numpy3D", to_type="nested_univ")
        ...
        X_test_df.set_index(['id_sample', 'id_time'], inplace=True)
        
        X_test_arr = convert_to(X_test_df, to_type="numpy3D")
        
        #X_test_nested = convert(X_test_arr, from_type="numpy3D", to_type="nested_univ")
...
for dataset in dataset_list:
    try:
        X_train, y_train, X_test, y_test = load_ucr_data(dataset)
        run_all_models(X_train, y_train, X_test, y_test, dataset)
    except Exception as e:
        print(f"[ERROR] Dataset {dataset} failed: {e}")

# === Save Step 1 Results ===
df_results = pd.DataFrame(results, columns=["dataset", "model", "kappa", "accuracy", "duration"])
df_results.to_csv("ucr_original_results.csv", index=False)
print("Original data completed. Results saved to ucr_original_results.csv")
